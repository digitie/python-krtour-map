...
- [ ] T-VN-41C — relay·reconciliation·consumer enable. **선행: `T-VN-41F1D-E`**(acceptance 정본 §배리어 5단계). **2026-09-06 재조사로 종전 서술을 정정한다** — "reconciliation은 구현이 남아 있다"는 거짓이었다. relay 넷(lease·retry·dead-letter·replay)이 `cache_target_outbox_repo.py`에 SQL 수준으로 있고, reconciliation 5-status 상태기계와 DB 대조(두 번 server-cursor scan + Merkle root)가 `cache_target_reconciliation_repo.py`에 있으며 라우터가 끝까지 부른다. 남은 것은 셋이다. (1) **런타임 결선** — 배포 Map 컨테이너에 `KOR_TRAVEL_MAP_API_CACHE_TARGET_SERVICE_PRINCIPALS`가 키 자체로 없어 service 표면이 401이고 relay 관계 19개가 전부 0행이다. (2) **enable 경계 구현** — PinVi가 production에서 sync enable을 스스로 거부하며 그 조문이 기다리는 "root-owned final C7 enable boundary"가 Manager에 없다. (3) **구조적 순환(소유자 판정 필요)** — 켜면 `environment_sha256`이 바뀌어 rebuild가 필요한데, Manager `require_rebuildable_mode`가 cache-target 값이 inert 기본값을 벗어나면 rebuild를 거부한다. 현 lifecycle에서 enable과 rebuild는 상호배타다. GC 실측 근거는 폐기 세대(head `0225`)의 것이고 러너는 `exit 2` stub이다. `1-a`/`1-b`/`1-c` 표기는 어느 정본에도 정의가 없다.
