...
- [ ] T-VN-41F1D-D2 — data-dependent Map/PinVi admin live E2E를 통과하고 receipt를 승격한다. **`T-VN-M01` 활성화 의존은 2026-09-06에 해소됐다** — kill-switch가 `true`이고 `POST /v1/admin/features`가 배포 스택에서 201을 낸다. 남은 것은 lane 자체의 완주다. 2026-09-05~06 실행에서 스펙은 **in-lane으로 통과했고**(main·recovery 각 `{"counts":{"passed":2},"result":"passed"}`) 이후 결함은 전부 **증거 계약** 쪽이었다 — 배포 스택에서만 드러나는 결함 열둘을 차례로 지났고 그때마다 탐지기를 남겼다. 마지막 하나(`seed` 직접 증거의 `summary_run_ids` 키)를 #1166이 닫았다.
